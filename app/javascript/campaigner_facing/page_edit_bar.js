...
import { isEqual, extend, each } from 'lodash';
...
const PageEditBar = Backbone.View.extend({
  el: '.page-edit-bar',
  ...
  addStepToSidebar: function($step) {
    const $ul = this.$('ul.page-edit-bar__step-list');
    const title = $step
  ...
  readData: function() {
    ...
    $('form.one-form').each((ii, form) => {
      const $form = $(form);
      const type = $form.data('type') || 'base';
  ...
  serializeForm: function($form) {
    ...
    each($form.serializeArray(), function(pair) {
      ...
      if (pair.name.endsWith('[]')) {
        const name = pair.name.slice(0, -2);
  ...
  showUnsavedAlert: function() {
    ...
    const $lastSaved = $('.page-edit-bar__last-saved');
    const noNotice =
